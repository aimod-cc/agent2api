 ...
 *
 * **脱敏分类本身仍然保留**：分类字典里照旧有「脱敏」一项，可以专门筛出来看。
 * 不过它现在装的是**不随请求量增长**的那几条（词表加载、默认词表补词、
 * 词表保存失败）—— 逐请求的「已脱敏命中 N 处」那行已经不再落库（只写终端），
 * 因为命中的逐条明细在请求日志里有了更直接的去处（见 `[Desensitize]` 的
 * 那条注释与 `core::desensitize::process_body_for_provider`）。
 *
 * ── 自动刷新的间隔从哪来 ────────────────────────────────────
