...
(() => {
   ...
   * 只标「需要关注的状态」；一切正常时给一个「启用」标签 ——
   * 否则整片都是空占位，反而看不出这个账号到底能不能用。
   * 启用 / 禁用这一对用**开关自身的语义**（与开关的 title 同一口径），
   * 而不是「正常 / 已禁用」—— 后者把开关状态与健康状态混成了两套词。
   */
  function accountTags(account) {
    const enabled = isEnabled(account);
    const tags = [
       // 没有转发能力的家：它的启用开关对转发没有意义，这里如实说明，
       // 而不是给它一个与事实不符的「启用」标签。判据是后端的 chatSupported
      ...
      enabled ? '' : statusTag('禁用', 'bad', '该账号已禁用，不参与转发'),
    ...
    if (!tags.length) return statusTag('启用', 'ok', '该账号可用，未触发限额');
    return tags.join('');
  }
