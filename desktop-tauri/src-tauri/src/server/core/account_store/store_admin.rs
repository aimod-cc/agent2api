...
use crate::server::core::account_store::priority::{
    renumber_consecutively, PriorityAssignment,
};
use crate::server::core::account_store::state::{json_number, StoredAccount};
...
impl AccountStore {
    ...
    /// 返回 `{changed, assignments}` 供启动日志展示。
    pub fn migrate_priorities(&self) -> Value {
        let _guard = self.guard();
        let mut state = self.load(&_guard);
        let values: Vec<i64> = state.accounts.iter().map(StoredAccount::priority).collect();
        let mut unique = values.clone();
        unique.sort_unstable();
        unique.dedup();
        if unique.len() == values.len() {
            return json!({ "changed": false, "assignments": [] });
        }

        state.accounts.sort_by_key(StoredAccount::order_key);
        let mut numbering: Vec<(String, String, i64)> = state
            .accounts
            .iter()
            .map(|item| (item.id().to_string(), item.name(), item.priority()))
            .collect();
        let assignments: Vec<PriorityAssignment> = renumber_consecutively(&mut numbering);
        // 只回写「确实改变」的账号：Node 的 renumberConsecutively 同样只在
        // from !== next 时赋值，所以「原本就没有 priority 字段」的账号迁移后
        // 依然没有该字段（它的生效值仍是默认 100）。若无差别地全量写回，
        // 用户的 accounts.json 会凭空多出一批 priority 字段。
