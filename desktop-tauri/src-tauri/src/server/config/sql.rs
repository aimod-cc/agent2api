...
//! （与 `desensitize` / `request_stats` 同一取舍）。
//!
...
///
/// 与 `desensitize` 那种「目标键在不在」的闸门不同 —— 这里**必须**用标记：
...
/// `logsNextId`、桌面设置 `desktopSettings`、脱敏状态 `desensitize`、两个迁移
/// 标记都在里面）。不排除它们的话，它们会被当成「配置里的未知字段」混进
